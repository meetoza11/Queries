...
where send_request_date >= getdate()-1
...
--SET last_mod_user = 'WALGREENS\ozadeven'
--where Profile_id = 5
--select * from msdb.dbo.sysmail_profile
--commit
...
SELECT recipients, send_request_date, sent_date, last_mod_date  FROM msdb.dbo.sysmail_sentitems
where recipients = 'deven.oza@walgreens.com'
...
--msdb.dbo.sysmail_delete_profile_sp @profile_id = [5]

--Begin Tran
--select count(*) from msdb.dbo.sysmail_allitems 
--Delete from msdb.dbo.sysmail_allitems where mailitem_id = 4493
--SELECT * FROM msdb.dbo.sysmail_allitems 
--commit
