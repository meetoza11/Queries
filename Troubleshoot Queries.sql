xp_logininfo 'brower\doza' --- run in this in SSMS
net user /domain doza -- run in cmd

xp_logininfo 'CORPINT\OG_SQL_DBA_Developer' --- run in this in SSMS
xp_logininfo 'NONPROD\SG_SQL_DBA_Developer' --- run in this in SSMS

-- PMI Folder
-- cd \PMI
ad u doza
ad g SQL_DBA_Developer


sp_a

sp_help_dba
-- When was last time server restarted?
SELECT sqlserver_start_time FROM sys.dm_os_sys_info

sp_spaceused '[dbo.table_name]'
sp_spaceused [bi.Chrysler_Dealer_Scorecard_Report_Fact]
...
-- to Find the number of cores, os name, version RDP to the server and go to the command prompt and run msinfo32 which will bring the all system related information.


sp_f  --- databases info
sp_a --- this one is for active Transaction

------ troubleshoot dead lock ------------ this 
...
---- once you see that xml_deadlock_report is enabled from the above query, run the below query to find out the dead lock
DECLARE @version int
...
-- Clear Wait Stats 
-- consider clearing and running wait stats query again after a few minutes,
-- especially if the server has been running for a long period
...
-- High runnable_tasks_count is a very good indication of CPU pressure
-- High pending_disk_io_count is a strong indication of I/O pressure
...
-- Top cached queries by Execution Count (SQL Server 2008 R2 SP1 and SQL Server 2012) 
SELECT TOP (25) qs.execution_count, qs.total_rows, qs.last_rows, qs.min_rows, qs.max_rows,
...
qs.min_elapsed_time, qs.max_elapsed_time, qs.last_elapsed_time,  qs.execution_count, qs.cached_time
FROM sys.procedures AS p WITH (NOLOCK)
INNER JOIN sys.dm_exec_procedure_stats AS qs WITH (NOLOCK)
ON p.[object_id] = qs.[object_id]
WHERE qs.database_id = DB_ID()
...
---- To see the index fragmentation on perticular table and index
DECLARE @db_id SMALLINT;
DECLARE @object_id INT;

SET @db_id = DB_ID(N'DW_Shawstore');
SET @object_id = OBJECT_ID(N'DW_Shawstore.Lease.Shaw_EXTMAST');
...
SELECT TOP(10) [type] AS [Memory Clerk Type], SUM(single_pages_kb) AS [Total Memory Usage, Kb] 
FROM sys.dm_os_memory_clerks WITH (NOLOCK)
GROUP BY [type]  
ORDER BY SUM(single_pages_kb) DESC OPTION (RECOMPILE);  --  single_pages_kb in SQL Server 2008 and 2008 R2
-- ORDER BY SUM(pages_kb) DESC OPTION (RECOMPILE);  -- pages_kb for SQL server 2012
...
----- following cursor kill sleeping ids
DECLARE @v_spid INT
